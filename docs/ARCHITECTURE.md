...
`Variant.status` (`IN_STOCK`/`PRE_ORDER`) is otherwise admin-set, not purely inventory-derived: `ProductForm`'s per-variant toggle sends it on create/update, so a variant with stock can still be listed as "Под заказ" (custom/made-to-order items) and one at zero stock stays `PRE_ORDER` until the admin restocks and flips it back — the zero-stock auto-flip only ever pushes it *into* `PRE_ORDER`, never back out.
