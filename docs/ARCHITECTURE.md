...
`ATTRIBUTE_KEY_OPTIONS`/`ATTRIBUTE_VALUE_OPTIONS` in `frontend/src/utils/attributeOptions.ts` are the single source of truth for both sides: `ProductForm`'s attribute key/value pickers and `FilterBar`'s color/size/volume filter pills draw from the exact same Russian value lists, so a value an admin picks is guaranteed filterable — the label *is* the stored value, no separate English filter-code layer. `ProductForm` still lets an admin type a value outside that list (a "Своё значение" escape hatch, e.g. a new print like "Леопардовый") since real product variety outruns any fixed enum; a variant loaded with a value not in the preset list opens in that free-text mode automatically. Values entered before this change (e.g. English codes like `black`, `500ml`) still display and filter fine internally, they just won't match the new Russian filter pills until the variant is re-saved through the dropdown.
