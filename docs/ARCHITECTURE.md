...
`GET /api/catalog/products` takes `search`, `category`, `color`, `size`, `volume` query params. `search` is an `ILIKE` on product name; `category` is an exact case-insensitive match on `product.category`; the attribute filters are native Postgres queries against the `variant.attributes` JSONB column (`attributes ->> 'color'`, etc.), case-insensitive, ANDed together. The frontend's color/size/volume filter pills are a hardcoded list of common values, not pulled from actual data. The category dropdown is the exception — there's still no facet/distinct-values endpoint, so `CatalogPage` derives it by running a second `useCatalog` call with the same search/color/size/volume filters but no category, and taking the distinct `category` values off whatever that returns; the dropdown only renders once that set is non-empty.

`ATTRIBUTE_KEY_OPTIONS`/`ATTRIBUTE_VALUE_OPTIONS` in `frontend/src/utils/attributeOptions.ts` are the single source of truth for both sides: `ProductForm`'s attribute key/value pickers and `FilterBar`'s color/size/volume filter pills draw from the exact same Russian value lists, so a value an admin picks is guaranteed filterable — the label *is* the stored value, no separate English filter-code layer, and no free-text entry in `ProductForm` (an attribute is a filter parameter, so it's dropdown-only, deliberately). A genuinely new value (a new print, a new size) means adding it to `attributeOptions.ts` — a one-line code change, not a runtime admin action — which is what keeps every stored attribute value filterable by construction. A variant loaded with a value outside the current list (legacy data, e.g. English codes like `black`, `500ml` from before this change) shows the value dropdown unselected/placeholder rather than silently keeping an unfilterable value; picking a real option on save is how it gets migrated.
