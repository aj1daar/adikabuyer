...
| CI e2e smoke | `.github/workflows/ci.yml` `e2e-smoke` job boots the real `docker-compose.prod.yml` stack and runs `scripts/e2e-smoke.sh` — Flyway migrations, RabbitMQ round-trips, gateway routing, cross-service re-pricing, Caddy headers. This is what would have caught the Spring Boot 4 runtime regressions before merge. |
