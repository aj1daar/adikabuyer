...
| Checkout re-pricing | `OrderService.checkout` re-resolves every cart line against catalog-service (`GET /api/catalog/variants/pricing`) and uses the catalog's price / name / SKU, never the client's. Unknown, inactive or `SOLD_OUT` variants are rejected (400/409), and stock is checked against each variant's total across all cart lines, so splitting one variant over several lines can't oversell it (`PRE_ORDER` excepted). |
