...
| Committed secrets | JWT secret and admin bcrypt hash removed from `application.yml` — now env-only (`APP_JWT_SECRET`, `APP_SECURITY_ADMIN_PASSWORD_HASH`), with visibly-fake dev fallbacks. `application-prod.yml` references them with no fallback, so an unset variable fails startup (`ProdProfileSecretsTest` guards this — Spring has no shell-style `${X:?msg}`, which would silently resolve to the literal `?msg` and make it the signing key); both services run `SPRING_PROFILES_ACTIVE=prod`. |
| Login rate-limit bypass | Caddy overwrites `X-Real-Ip` **and** `X-Forwarded-For` with the real TCP peer (`AuthController` keys on `X-Real-Ip` — the gateway rewrites `X-Forwarded-*` per CVE-2025-41235 but passes `X-Real-Ip` through). Global 20/min backstop across all clients. Limits are configurable (`app.security.login.*`); prod defaults 5/ip + 20 global, local dev raises them so testing doesn't lock you out. Gateway `trusted-proxies` set so it doesn't accept forwarded headers from untrusted peers. |
...
| Telegram registration | Subscribing to order notifications hands out customer PII, so `TelegramAdminService` compares the registration password in constant time, `RegistrationAttemptLimiter` allows 5 guesses per chat and 30 across all chats per hour (`app.telegram.registration-*`), and `TelegramUpdatePoller` alerts every existing admin chat when a new one registers. |
| Checkout input | `CartDto` only accepts a phone made of digits and `+ ( ) -` and a region of exactly `Бишкек` or `самовывоз` (400 otherwise); `OrderService` flattens control characters in the name/phone/region to spaces, so a crafted name can't inject fake lines ("Итого: 0 KGS") into the Telegram notification. |
...
| CI e2e smoke | `.github/workflows/ci.yml` `e2e-smoke` job boots the real `docker-compose.prod.yml` stack and runs `scripts/e2e-smoke.sh` — Flyway migrations, RabbitMQ round-trips, gateway routing, cross-service re-pricing, checkout region validation, the 256KB order body cap, Caddy headers. This is what would have caught the Spring Boot 4 runtime regressions before merge. |
...
| Checkout re-pricing | `OrderService.checkout` re-resolves every cart line against catalog-service (`GET /api/catalog/variants/pricing`) and uses the catalog's price / name / SKU / attributes, never the client's — so the order and its Telegram message can't carry client-invented attribute values or an arbitrarily large attributes blob. Unknown, inactive or `SOLD_OUT` variants are rejected (400/409), and stock is checked against each variant's total across all cart lines, so splitting one variant over several lines can't oversell it (`PRE_ORDER` excepted). |
| Supply chain | MinIO pinned to a release digest (pulled from `quay.io/minio/minio` — the Docker Hub `minio/minio` repository now refuses pulls), `appleboy/ssh-action` to a commit SHA; deploy force-recreates the gateway so it can't hold a stale IP for a rebuilt catalog/order container. |
