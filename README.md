...
- Catalog filter pills for color/size/volume are a hardcoded list on the frontend (`utils/attributeOptions.ts`), not derived from real product data — but the admin's attribute value picker draws from that same list, so normal use stays filterable by construction. A variant given a custom value via the "Своё значение" escape hatch (e.g. a new print not yet in the list) is unfilterable until that value is added to the shared list. The category filter is the exception — it's derived from real data, but only via a second unfiltered fetch on every page load; there's still no dedicated distinct-values endpoint.
