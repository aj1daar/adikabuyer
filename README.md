...
- Catalog filter pills for color/size/volume are a hardcoded list on the frontend (`utils/attributeOptions.ts`), not derived from real product data — but the admin's attribute picker is dropdown-only from that exact same list (no free-text entry), so every stored attribute value is guaranteed filterable. A brand-new value (a new print, a new size) means adding it to `attributeOptions.ts` in code. The category filter is the exception — it's derived from real data, but only via a second unfiltered fetch on every page load; there's still no dedicated distinct-values endpoint.
